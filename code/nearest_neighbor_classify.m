...
distance_table = vl_alldist2(train_image_feats', test_image_feats');
unique_labels = unique(train_labels);
labels_size = size(unique_labels, 1);
[~, indices] = sort(distance_table, 1);
predicted_categories = cell(M, 1);
for i = 1 : M
    most_freq_idx = 0;
    freq = 0;
    for j = 1 : labels_size
        nearest_labels = train_labels(indices(1 : num_of_NBRs, i));
        temp = sum(strcmp(unique_labels(j), nearest_labels));
        if (temp > freq)
            most_freq_idx = j;
            freq = temp;
        end
    end
    predicted_categories{i, 1} = unique_labels{most_freq_idx};
end
end

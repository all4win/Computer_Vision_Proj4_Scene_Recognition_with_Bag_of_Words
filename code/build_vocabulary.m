...
sampled_feats = zeros(128, N * feats_num_each);
for i = 1 : N
    single_img = im2single(imread(image_paths{i}));
    [~, feats] = vl_dsift(single_img, 'Step', 10);
    selected_col_idx = unique(randi(size(feats, 2), [1, ceil(1.2 * feats_num_each)]));
    feats = feats(:, selected_col_idx);
    sampled_feats(:, feats_num_each * (i - 1) + 1 : feats_num_each * i) = feats(:, 1 : feats_num_each);
end
[centers, ~] = vl_kmeans(sampled_feats, vocab_size);
